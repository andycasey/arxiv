...
\noindent Peer-reviewed publications are the most common measure of productivity in \change{many specialties of science and technology}. The \arxiv\cite{Ginsparg:2011} ({https://arxiv.org}) is a service for the distribution of research manuscripts (pre-prints) prior to their appearance in a journal. The presence of a pre-print on \arxiv\ does not ensure that the contents have passed peer-review, but most material on \arxiv\ eventually goes through peer-review because it is now standard in many research specialties to post to \arxiv\ either during or after the peer-review process \cite{Lariviere:2014}. 
\change{The fraction of peer-reviewed publications that are also posted to the \arxiv\ increases each year\cite{Davis:2007} but not all academic outputs (peer reviewed publications or otherwise) will appear on the \arxiv. Fundamental reports, findings from inquiries or research groups, and many other texts that are only published in peer-reviewed publications will never appear on the \arxiv. In the specialisations we investigate here (physics, mathematics, astronomy, computer science, and other STEM-related fields), the fraction of peer-reviewed literature that also appears on \arxiv\ is estimated to vary from 18.5\% in mathematics to nearly 100\% in high energy physics\cite{Davis:2007,GentilBeccot:2009}. Although the fraction may vary between discplines, within a given specialisation the number of pre-prints provides a proxy measure of the number of peer-reviewed publications, which we can use to examine changes in each specialty as a function of time.}
...
The COVID-19 pandemic appears to have had minimal impact on collective research productivity to date, \change{and that is before accounting for the tremendous work in scientific communication and contributions to public policy throughout the pandemic.} However, some significant changes were noted within specific areas of research. The largest persistent decline in pre-prints has occurred in {general physics (arXiv primary category \texttt{physics}), largely due to a decline in  pre-prints posted to computational and applied physics areas within the \texttt{physics} primary category. As of mid-2022, this steady decline continues.} Amongst individual fields the largest \emph{relative} decline in a single field is explainable by a high energy lattice physics conference that was cancelled in 2020. In this study we have only focussed on the quantity of \change{pre-prints} as a measure for productivity, and not their quality. While citations are the most used measure of impact of academic publications, that metric becomes a more biased statistic when many related pre-prints are all being posted nearly at the same time\cite{Fassin:2021}. The relatively long timescales of academic research would suggest that the full impact on academic productivity due to the COVID-19 pandemic is yet to be seen. The methodology and results presented here provide a quantitative framework for future investigations of the longer term impacts of the pandemic.
...
\subsection*{Identifying research specialisations of pandemic-related pre-prints posted by new authors in \texttt{q-bio}}

\change{There were 234 pre-prints posted to the \texttt{q-bio} primary category in 2020 which include pandemic-related terms, and where no author had never appeared on the \texttt{q-bio} pre-print before. We investigated every pre-print to extract the affiliation of the lead author, and used this information to search the internet for the author's website. From this information we identified their research specialisations using their own words, or from their social media pages if no website could be found. In situations where an author's specialisation crossed multiple fields (e.g., mathematics and biology) we took a conservative approach and described the author as a biologist. However, we found that only 3\% (8/234) were led by established biologists who had not appeared in \texttt{q-bio} pre-prints before. About half (45\%; 107/234) were led by mathematicians or physicists, with another 26\% (61/234) led by engineers, computer scientists, or economists.}
